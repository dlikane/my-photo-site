...
- **Nothing in this subsystem touches local disk, on either side.** Images enter purely via drag-drop or a file picker (works with Explorer/Finder, or the Gallery/Photos picker on mobile) and live only in an in-memory, session-scoped image pool (`state/imagePoolStore.tsx`), keyed by a SHA-256 content hash rather than a path, metadata fingerprint, or random ID — so re-dropping the same original file (e.g. after reopening a layout) always resolves to the same key regardless of name/mtime. "Export"/"Open" (named to avoid implying persistence that doesn't exist — deliberately not called "Save") are plain browser file download/upload of a layout-only JSON (no image bytes); "Render" is the one operation that calls the backend, POSTing the doc + the actual image bytes to render the final JPEG.
