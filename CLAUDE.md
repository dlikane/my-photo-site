...
### Collage Studio: no backend at all, nothing ever leaves the device

- `src/collage-studio/` implements a browser-based photo collage editor, merged into this site as the `/collage-studio` route. `src/components/Main.jsx` special-cases that path to skip the site's `Header`/`Footer` chrome.
- **Nothing in this subsystem touches local disk or talks to any backend.** Images enter purely via drag-drop or a file picker (works with Explorer/Finder, or the Gallery/Photos picker on mobile) and live only in an in-memory + IndexedDB-persisted, device-local image pool (`state/imagePoolStore.tsx`), keyed by a SHA-256 content hash rather than a path, metadata fingerprint, or random ID — so re-dropping the same original file (e.g. after reopening a layout) always resolves to the same key regardless of name/mtime. "Export" (layout-only JSON) and "Open" are plain browser file download/upload; "Render" (the final JPEG) is also fully client-side now — `model/canvasExport.ts` redraws the doc at full `doc.canvas.width/height` resolution onto an off-screen `<canvas>` (reusing the exact same layout/crop/draw code as the live preview, just unscaled) and calls `canvas.toBlob()`. There used to be a render backend (first a local-only Python/Pillow process, briefly a Vercel function backed by Blob storage); both were replaced with this client-side approach so Render works from any device with zero network dependency. If canvas-quality output ever isn't good enough, revisit — see git history for both prior approaches.
